...
import { Question, Option } from '../../../api/types/apiResponses/api-response-quizzes';
    ...
    return (
      <div className="flex flex-col items-center justify-center h-auto mb-5">
        <div className="pt-10 pb-10 flex justify-center items-center">
          <h2 className="text-xl font-semibold mt-3">Please login to solve quizzes</h2>
          <Link to="/login" className="ml-2 mt-3.5 text-blue-600 underline">
            Login
          </Link>
        </div>
      </div>
    ...
    <div className="">
      ...
      <div className=" mx-auto px-4">
        {quizzes?.length ? (
          ...
          <p className="text-lg text-center">No quizzes available for this lesson.</p>
        )}
      </div>
    </div>
  );
