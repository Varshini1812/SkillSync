...
import { Tooltip } from '../../material-overrides';
      ...
      <div className="md:ml-3 mb-8">
        <ul>
          {visibleCommentsList?.map((item, index) => {
            return (
              <DiscussionListEl updated={updated} setUpdated={setUpdated} {...item} key={index} />
            );
          })}
        </ul>
        {shouldShowViewMore && (
          <button className="text-blue-gray-400 ml-4 mt-1 underline" onClick={handleViewMore}>
            View More
          </button>
        )}
        {shouldShowShowLess && (
          <button className="text-blue-gray-400 ml-4 mt-1 underline" onClick={handleShowLess}>
            Show less
          </button>
        )}
      </div>
