...
export const rejectInstructorRequest = (instructorId: string, reason: string) => {
  return rejectRequest(END_POINTS.REJECT_INSTRUCTOR_REQUESTS, instructorId, reason);
};
...
export const blockInstructors = (instructorId: string, reason: string) => {
  return blockInstructor(END_POINTS.BLOCK_INSTRUCTORS, instructorId, reason);
};
