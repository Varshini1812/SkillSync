...
export const addCourseService = async (endpoint: string, courseInfo: FormData) => {
  const response = await api.post(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}`, courseInfo);
  return response;
};
...
export const editCourseService = async (
  ...
  const response = await api.put(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}/${courseId}`, courseInfo);
  return response;
};
...
export const getAllCoursesService = async (endpoint: string) => {
  const response = await axiosInstance.get(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}`);
  return response;
};

export const getIndividualCourseService = async (endpoint: string, courseId: string) => {
  const response = await axiosInstance.get(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}/${courseId}`);
  return response;
};
