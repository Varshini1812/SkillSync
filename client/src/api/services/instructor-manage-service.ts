...
export const acceptRequest = async (endpoint: string, instructorId: string) => {
  const response = await api.patch(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}/${instructorId}`);
  return response;
};

export const rejectRequest = async (endpoint: string, instructorId: string, reason: string) => {
...
export const blockInstructor = async (endpoint: string, instructorId: string, reason: string) => {
  const response = await api.patch(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}`, {
    instructorId,
    reason,
  });
  return response;
...
export const unblockInstructor = async (endpoint: string, instructorId: string) => {
  const response = await api.patch(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}/${instructorId}`);
  return response;
};
