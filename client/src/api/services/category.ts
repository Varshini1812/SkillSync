...
export const addCategoryService = async (
  ...
  const response = await api.post(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}`, categoryInfo);
  return response;
};
...
export const getCategoryByIdService = async (endpoint: string, categoryId: string) => {
  const response = await api.get(`${CONFIG_KEYS.API_BASE_URL}/${endpoint}/${categoryId}`);
  return response.data;
};
